...
exports.retentionCleanup = onSchedule('every day 02:00', async () => {
  const policies = await db.collectionGroup('retentionPolicies').where('isActive', '==', true).get();
  for (const policyDoc of policies.docs) {
    ...
    const orgRef = policyDoc.ref.parent.parent;
    if (!orgRef) continue;
    ...
    for (const config of queryConfigs) {
      ...
      expiredMessages.docs.forEach((msgDoc) => {
        if (policy.action === 'archive') {
          ...
          batch.set(orgRef.collection(config.archiveCollection).doc(archiveId), {
            ...msgDoc.data(),
    ...
    await orgRef.collection('retention_cleanup_logs').add({ ruleId: policyDoc.id, ruleName: category, affected, status: 'completed', timestamp: admin.firestore.FieldValue.serverTimestamp() });
