...
export default function ProfileSettingsModal({
  ...
  setToolPreferences,
  user,
  customTags // 👈 RECEIVES GLOBAL TAGS
}) {
  ...
  const currentTags = toolPreferences?.quickTags || ['#Approved', '#Reviewing', '#ActionRequired', '#Noted'];
          ...
          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-3 cursor-pointer p-2 hover:bg-white rounded-lg transition-colors">
                ...
                <span className="text-[14px] font-medium text-slate-700"><i className="fa-solid fa-hashtag mr-2 text-[#54656f]"></i> Quick Workflow Tags</span>
              </label>
              <label className="flex items-center gap-3 cursor-pointer p-2 hover:bg-white rounded-lg transition-colors">
            ...
            {/* 👇 OFFICIAL WORKFLOW TAG PICKER */}
            {toolPreferences.react !== false && (
              <div className="mt-4 pt-4 border-t border-slate-200">
                 <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2 px-1">Menu Quick Tags (Max 4)</label>
                 <div className="flex flex-col gap-2">
                   {[0, 1, 2, 3].map(i => (
                      <select key={i} value={currentTags[i] || ''} onChange={e => {
                            const newTags = [...currentTags]; newTags[i] = e.target.value;
                            setToolPreferences(prev => ({...prev, quickTags: newTags.filter(t => t.trim() !== '')}));
                         }} 
                         className="w-full text-[12px] font-bold bg-white border border-slate-200 rounded-lg shadow-sm p-2 outline-none focus:ring-2 focus:ring-[#008069]/20"
                      >
                         <option value="">None</option>
                         {customTags.map(t => <option key={t.id} value={t.label}>{t.label}</option>)}
                      </select>
                   ))}
                 </div>
              </div>
