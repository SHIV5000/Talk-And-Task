...
const MessageBubble = React.memo(({
  ...
  const bubbleWidthClass = isThreadView ? 'max-w-full' : msg.isTask ? 'max-w-[70%]' : 'max-w-[min(78%,720px)]';
  const taskShellClass = msg.isTask ? `task-card-v19 ${msg.isMine ? 'sent' : ''} border-2 border-indigo-100 border-l-[6px] rounded-[20px]` : '';
  ...
  return (
    <div id={`msg-${msg.id}`} className={`w-full flex ${msg.isTask ? 'justify-start' : msg.isMine ? 'justify-end' : 'justify-start'} ${isThreadView ? 'mb-4' : 'msg-row-spacing'} transform-gpu group/msg ${isUnreadHighlight || isHighlighted ? 'highlight-flash' : ''} ${menuOpen ? 'relative z-[120]' : 'relative z-[1]'}`}>
      ...
      <div className={`${bubbleWidthClass} ${taskShellClass} message-bubble-v19 w-fit min-w-0 bg-white rounded-[24px] shadow-sm border border-slate-100 ${getBorderColor()} border-l-4 px-[18px] py-3 relative ${msg.isMine ? 'message-bubble-sent' : 'message-bubble-received'} break-words flex flex-col`}>
        ...
        <div className="flex-1 w-full">
            ...
            {menuOpen && (
              <div ref={menuRef} className="absolute top-8 right-2 z-[120] bg-white rounded-xl shadow-lg border border-slate-200 py-2 w-48 animate-in fade-in slide-in-from-top-2" onClick={(e) => e.stopPropagation()}>
                ...
                {!msg.isTask && !isThreadView && <button onClick={() => { setMenuOpen(false); setActiveTaskSidebar?.(null); setActiveReplies(msg); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600"><i className="fa-solid fa-reply w-5"></i> Reply in Replies</button>}
        ...
        <div className="mt-3 pt-2.5 border-t border-slate-100 flex flex-wrap items-end justify-between gap-3 w-full">
            <div className="flex flex-wrap items-center gap-1.5 flex-1">
                {!msg.isTask && Object.entries(msg.reactions || {}).map(([tagLabel, users]) => {
