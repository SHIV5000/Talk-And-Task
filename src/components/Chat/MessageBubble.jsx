          ...
          {isEditingThis ? (
            <div className="flex flex-col gap-2 my-1" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setEditingMessageId(null)} className="text-xs text-text-secondary font-semibold px-3 py-1 hover:bg-gray-100 rounded">Cancel</button>
                <button onClick={() => handleSaveEdit(msg)} className="text-xs bg-primary text-white px-4 py-1 rounded font-semibold hover:bg-primary-hover">Save</button>
              </div>
            </div>
          ) : (
            <>
              ...
              {msg.isPrivateMention && <div className="text-xs font-semibold flex items-center gap-1 mb-2 text-purple-700"><i className="fa-solid fa-lock"></i> {msg.text?.startsWith('[Forwarded') ? 'FORWARDED DM' : 'PRIVATE'}</div>}
              {msg.isPrivateForward && <div className="text-xs font-semibold flex items-center gap-1 mb-2 text-purple-700"><i className="fa-solid fa-lock"></i> Private from {msg.forwardedFromGroup}</div>}
              {msg.fileUrl ? (
                <div className="flex flex-col gap-1 my-1">
                  {msg.fileType?.startsWith('image/') ? (
                    <img src={msg.fileUrl} alt="Shared" className="rounded max-w-full max-h-64 object-cover cursor-pointer" onClick={(e) => { e.stopPropagation(); window.open(msg.fileUrl, '_blank'); }} />
                  ) : (
              ...
              {msg.isTask && (
                <div className="mt-2 bg-gray-50 p-2 rounded flex flex-col gap-1 border border-gray-100">
