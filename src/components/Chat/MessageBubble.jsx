...
const MessageBubble = React.memo(({
  ...
  const handlePrintTrail = (e) => {
    ...
    const tableRows = (msg.taskData.trail || []).map(t => [
        t.time, 
        (t.by || '').split('@')[0], 
        t.action, 
        t.comment || t.fileName || '-'
    ]);
              ...
              <div className={`mt-2 border rounded-xl overflow-hidden shadow-sm transition-all ${isTaskCompleted ? 'bg-slate-50 border-slate-200 opacity-95' : 'bg-white border-slate-200 hover:shadow-md'}`}>
                ...
                {isTaskExpanded && (
                  <div className="bg-slate-50 border-t border-slate-200 p-3 animate-in slide-in-from-top-2">
                    ...
                    {/* Action Bar: Disabled if Completed */}
                    {canEditTask ? (
                    ...
                    {/* 👇 Management Actions Row (Print, Archive, SuperAdmin Reopen) */}
                    <div className="flex justify-between items-center mt-4 pt-3 border-t border-slate-200">
                        ...
                        <div className="flex gap-2">
                            ...
                            {isCreator && (
                                <button onClick={handleInlineArchive} className="text-[11px] font-bold text-slate-500 hover:text-danger flex items-center gap-1.5 transition-colors bg-white px-2 py-1 rounded border border-slate-200 shadow-sm hover:border-red-200 hover:bg-red-50">
