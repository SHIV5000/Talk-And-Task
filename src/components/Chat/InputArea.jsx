...
export default function InputArea({
  ...
  return (
    <div className="bg-[#f0f2f5] px-3 md:px-4 py-3 shrink-0 z-10 flex flex-col gap-2 safe-bottom relative w-full">
      ...
      {/* File rename card (multi-file) */}
      {showFileRename && pendingFiles.length > 0 && (
        <div className="bg-white border border-[#00a884] shadow-lg rounded-2xl p-4 animate-in slide-in-from-bottom-2 z-20 space-y-3">
          {pendingFiles.map((pf) => (
            <div key={pf.id} className="flex items-start gap-3 border-b border-slate-100 pb-3 last:border-0 last:pb-0">
              ...
              <div className="flex-1 space-y-1.5">
                ...
                <input
                  type="text"
                  value={pf.caption}
                  onChange={(e) => setPendingFiles(prev => prev.map(f => f.id === pf.id ? { ...f, caption: e.target.value } : f))}
              ...
              <button
                onClick={() => { setPendingFiles(prev => prev.filter(f => f.id !== pf.id)); if (pendingFiles.length === 1) setShowFileRename(false); }}
                className="text-slate-400 hover:text-red-500 p-2"
              ><i className="fa-solid fa-trash-can"></i></button>
            </div>
          ))}
          <div className="flex justify-end gap-2 pt-1">
            <button onClick={() => { setPendingFiles([]); setShowFileRename(false); }} className="text-slate-500 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-slate-100 transition-colors">Cancel</button>
            <button
              onClick={() => { if (pendingFiles.length === 0) { setShowFileRename(false); return; } pendingFiles.forEach(pf => uploadFileDirectly(pf)); setShowFileRename(false); }}
              className="bg-[#008069] text-white px-5 py-2 rounded-xl text-sm font-bold shadow-sm hover:bg-[#006e5a] transition-colors"
            >Upload All ({pendingFiles.length})</button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-end gap-2">
        ...
        <button className="w-[42px] h-[42px] flex items-center justify-center text-[#54656f] hover:text-[#111b21] transition-colors shrink-0 text-[22px]" onClick={() => fileInputRef.current.click()} disabled={isUploading}>
          <i className="fa-solid fa-plus"></i>
        </button>

        {/* Emoji picker */}
        <div className="relative shrink-0" ref={emojiPickerRef}>
          <button onClick={() => setEmojiPickerOpen(!emojiPickerOpen)} className="w-[42px] h-[42px] flex items-center justify-center text-[#54656f] hover:text-[#111b21] transition-colors text-[22px]" title="Emoji">
        ...
        <div className="flex-1 bg-white rounded-lg flex items-end shadow-sm overflow-hidden">
          <textarea
            ...
            className="bg-transparent flex-1 outline-none text-[15px] text-[#111b21] resize-none py-[10px] px-4 w-full"
            style={{ minHeight: '42px', maxHeight: '120px' }}
            value={inputText}
            onPaste={handlePaste}
            onChange={(e) => { setInputText(e.target.value); handleTypingEvent(); e.target.style.height = 'auto'; e.target.style.height = (e.target.scrollHeight < 120 ? e.target.scrollHeight : 120) + 'px'; }}
            onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendOfflineAware(); } }}
          />
        </div>

        {/* Schedule button */}
        <button onClick={() => { if (!inputText.trim()) return alert("Type a message first, then schedule it."); setPendingScheduledText(inputText.trim()); setActiveModal('schedule_send'); }} className="shrink-0 w-[42px] h-[42px] flex justify-center items-center text-[#54656f] hover:text-amber-500 transition-colors" title="Schedule this message">
        ...
        {/* Send button */}
        {inputText.trim() ? (
          <button onClick={handleSendOfflineAware} className="shrink-0 w-[42px] h-[42px] flex justify-center items-center text-[#54656f] hover:text-[#00a884] transition-colors">
            <i className="fa-solid fa-paper-plane text-[22px]"></i>
          </button>
        ) : (
          <button className="shrink-0 w-[42px] h-[42px] flex justify-center items-center text-[#54656f] opacity-50 cursor-not-allowed">
            <i className="fa-solid fa-paper-plane text-[22px]"></i>
          </button>
        )}
      </div>
