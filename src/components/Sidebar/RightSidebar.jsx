...
export default function RightSidebar({
  ...
  const renderTaskCard = (task) => {
    ...
    return (
      <div key={task.id} onClick={handleClick} className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm cursor-pointer hover:shadow-md transition-shadow mb-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
              task.taskData.priority === 'High'   ? 'bg-red-100 text-red-700' :
              task.taskData.priority === 'Medium' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>
            ...
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
              ...
              task.taskData.status === 'In Progress' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'}`}>
          ...
          <div className="flex items-center gap-2">
            <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
              <div className={`h-full rounded-full ${
                task.taskData.status === 'Completed' ? 'bg-teal-500 w-full' :
                task.taskData.status === 'In Progress' ? 'bg-indigo-500 w-1/2' : 'bg-amber-500 w-1/4'}`}></div>
            </div>
  ...
  return (
    <div className="absolute right-0 md:relative w-80 h-full bg-surface border-l border-gray-200 flex flex-col shrink-0 z-40 animate-in slide-in-from-right shadow-lg">
      <div className="h-[59px] bg-white border-b border-gray-200 flex items-center justify-between px-4 shrink-0 z-10 safe-top">
        ...
        <button onClick={() => setShowRightSidebar(false)} className="w-10 h-10 rounded-full hover:bg-gray-100 text-text-secondary transition-colors flex items-center justify-center text-[19px]">
