...
export default function AdminPanel({
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (taskStatusFilter !== 'All')
      tasks = tasks.filter(
        (t) => t.taskData?.status === taskStatusFilter
      );
    if (taskGroupFilter)
  ...
  const universalLogs = useMemo(() => {
    // Merge all log types including LOGIN, LOGOUT, REACTION, TASK_*, MESSAGE_*
    let logs = [...(filteredAuditLogs || [])];
    if (logSearchQuery.trim()) {
      const q = logSearchQuery.toLowerCase();
      logs = logs.filter((l) => {
        const userName = (
          (dbUsers || []).find((u) => u.email === l.user)?.name || ''
        ).toLowerCase();
  ...
  const overviewMetrics = useMemo(() => {
    ...
    const recentMsgs = (messages || []).filter(
      (m) => (m.timestamp?.toMillis?.() || 0) >= cutoff
    );
    const recentTasks = recentMsgs.filter((m) => m.isTask);
    const overdueTasks = allTasks.filter(
      (t) =>
        t.taskData?.status !== 'Completed' &&
        new Date(t.taskData?.deadline) < new Date()
    );
    return {
      totalMessages: recentMsgs.length,
      activeTasks: recentTasks.filter(
        (t) => t.taskData?.status !== 'Completed'
      ).length,
      completedTasks: recentTasks.filter(
        (t) => t.taskData?.status === 'Completed'
      ).length,
      activeUsers: (dbUsers || []).filter(
        (u) => u.lastActive?.toMillis?.() >= cutoff
      ).length,
  ...
  const recentAuditFeed = useMemo(
    () =>
      (filteredAuditLogs || []).slice(0, 15).map((log) => ({
        ...log,
        userName:
          (dbUsers || []).find((u) => u.email === log.user)?.name || 'System',
      })),
          ...
          <div className="flex flex-col gap-6 p-4 md:p-6 overflow-y-auto custom-sidebar-scroll h-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div
                ...
                className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all cursor-pointer group"
              >
              ...
              <div
                ...
                className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-amber-300 transition-all cursor-pointer group"
              >
                <div className="flex items-center justify-between">
                  <div>
              ...
              <div
                ...
                className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-emerald-300 transition-all cursor-pointer group"
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider group-hover:text-emerald-500 transition-colors">
                      Completed
                    </p>
                    <p className="text-2xl font-extrabold text-slate-800 mt-1">
              ...
              <div
                ...
                className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm hover:shadow-md hover:border-teal-300 transition-all cursor-pointer group"
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider group-hover:text-teal-500 transition-colors">
                      Active Users
                    </p>
                    <p className="text-2xl font-extrabold text-slate-800 mt-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
                <div>
                  ...
                  <p className="text-2xl font-extrabold text-rose-600 mt-1">
                    {overviewMetrics.pendingApprovals}
                  </p>
                </div>
                <button
                  onClick={() => setActiveTab('people')}
              ...
              <div
                ...
                className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow cursor-pointer"
              >
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">
                    Overdue Tasks
                  ...
                  <p className="text-2xl font-extrabold text-rose-600 mt-1">
                    {overviewMetrics.overdueTasks}
                  </p>
                </div>
                <button className="text-xs font-bold text-rose-600 bg-rose-50 px-4 py-2 rounded-lg hover:bg-rose-100 transition-colors">
                  View All
                </button>
              </div>
            </div>
              ...
              <div className="divide-y divide-slate-100 overflow-y-auto custom-sidebar-scroll">
                {recentAuditFeed.length === 0 ? (
                  ...
                  recentAuditFeed.map((log) => (
                    ...
                    >
                      <span
                        className={`w-9 h-9 rounded-full flex items-center justify-center text-xs shadow-sm ${
                          log.type?.startsWith('TASK_')
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] text-slate-700 truncate leading-snug">
                          ...
                          <span className="font-medium">{stripHtml(log.content)}</span>
                        </p>
                        <p className="text-[11px] font-bold text-slate-400 mt-0.5">
                          {log.dateString} at {log.time}
                        </p>
                      </div>
                    </div>
                  ))
              ...
              {showAddUser && (
                <div className="p-5 bg-slate-50 border-b border-slate-200 flex flex-wrap gap-4 items-end">
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={newUserApprove}
                      onChange={(e) => setNewUserApprove(e.target.checked)}
                  ...
                  <button
                    ...
                    className="bg-teal-500 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-teal-600"
                  >
                    Save
                  </button>
                  <button
                    onClick={() => setShowAddUser(false)}
                    className="bg-slate-200 text-slate-600 px-5 py-2 rounded-lg text-sm font-bold hover:bg-slate-300"
                  >
                    Cancel
                  </button>
                </div>
              ...
              {selectedUsers.size > 0 && (
                <div className="px-5 py-3 bg-indigo-50 border-b border-indigo-100 flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        await Promise.all(
                          Array.from(selectedUsers).map((uid) =>
                            updateDoc(doc(db, 'users', uid), { isAdmin: false })
                          )
                        );
                        setSelectedUsers(new Set());
                      }}
                    ...
                    <button
                      onClick={async () => {
                        ...
                        await Promise.all(
                          Array.from(selectedUsers).map((uid) =>
                            updateDoc(doc(db, 'users', uid), { isArchived: true })
                          )
                        );
                        setSelectedUsers(new Set());
                      }}
                      className="px-3 py-1.5 bg-slate-500 text-white text-xs font-bold rounded-lg hover:bg-slate-600"
                    >
                      Archive
                    </button>
                    <button
                      onClick={() => {
                        const sel = filteredUsers.filter((u) => selectedUsers.has(u.uid));
                        const csv =
                          'Name,Email,Approved,Admin\n' +
                          sel
                            .map(
                              (u) =>
                                `"${u.name}","${u.email}","${u.isApproved}","${u.isAdmin}"`
                            )
                            .join('\n');
                  ...
                  <button
                    ...
                    className="ml-auto text-xs font-bold text-slate-500 hover:text-rose-600"
                  >
                    Clear
                  </button>
                </div>
                    ...
                    {filteredUsers.map((u, idx) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-3 text-center text-[11px] text-slate-500">
                          {u.lastActive?.toDate
                            ? new Date(u.lastActive.toDate()).toLocaleDateString()
                            : '—'}
                        </td>
                      </tr>
                    ))}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  ...
                  >
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-slate-200 p-3 rounded-xl text-sm outline-none focus:border-indigo-500"
                        required
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="text-sm"
                      />
                      {groupPicUploadProgress > 0 && (
                        <span className="text-xs font-bold text-indigo-600 ml-2">
                          {Math.round(groupPicUploadProgress)}%
                        </span>
                      )}
                    </div>
                    <div>
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => setGroupForm({ ...groupForm, members: [] })}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                        {dbUsers.map((u) => (
                          <label
                            key={u.uid}
                            className="flex items-center gap-2 text-sm p-2 bg-white border border-slate-200 rounded-lg cursor-pointer"
                          >
                            <input
                              type="checkbox"
                    ...
                    <div className="flex justify-end gap-3">
                      <button
                        ...
                        className="px-4 py-2 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50"
                      >
                        Cancel
                      </button>
                      <button
              ...
              <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 overflow-y-auto">
                {groups.map((g) => (
                  <div
                    key={g.id}
                    className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:border-indigo-300 transition-all"
                  >
                      ...
                      {(g.members || []).slice(0, 6).map((email) => {
                        ...
                        return (
                          <MemoizedAvatar
                            ...
                            url={u?.profilePicUrl}
                            name={u?.name || 'User'}
                            sizeClass="w-6 h-6 border-2 border-white shadow-sm"
                          />
                        );
                      })}
                      {(g.members || []).length > 6 && (
                        <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center text-[9px] font-bold text-slate-600 border-2 border-white shadow-sm">
                          +{g.members.length - 6}
                        </div>
                      )}
                    </div>
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-slate-50 text-slate-600 py-2 rounded-lg text-xs font-bold border border-slate-200 hover:bg-indigo-50 hover:text-indigo-600"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 bg-white text-rose-500 py-2 rounded-lg text-xs font-bold border border-rose-200 hover:bg-rose-50"
                      >
          ...
          <div className="flex flex-col gap-4 p-4 md:p-6 overflow-y-auto custom-sidebar-scroll h-full">
            <div className="flex flex-wrap gap-3 items-center justify-between">
              <h2 className="font-bold text-lg text-slate-800">
                <i className="fa-solid fa-list-check text-indigo-600 mr-2"></i>Tasks
              </h2>
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <input
                  ...
                  className="border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold w-48"
                />
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredTasks.map((task) => {
                      ...
                      const groupName =
                        ...
                        (task.groupId === 'direct' ? 'Direct' : 'Unknown');
                      return (
                        <React.Fragment key={task.id}>
                          ...
                          >
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <i
                                  className={`fa-solid fa-flag text-xs ${
                            ...
                            <td className="px-4 py-3">
                              {editingTaskId === task.id ? (
                                <select
                                  ...
                                  className="border border-slate-200 rounded p-1 text-xs font-bold"
                                ...
                                >
                                  {task.taskData?.status || 'Pending'}
                                </span>
                              )}
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex -space-x-1.5">
                                {(task.taskData?.assignees || [])
                                  .slice(0, 3)
                                  .map((email) => {
                                    const u = dbUsers.find((u) => u.email === email);
                                    return (
                            ...
                            <td className="px-4 py-3 text-xs font-medium">
                              {new Date(task.taskData?.deadline).toLocaleDateString()}
                            </td>
                                ...
                                <div className="flex flex-wrap gap-4 items-end">
                                  <div>
                                    ...
                                    <input
                                      ...
                                      className="border border-slate-200 rounded p-1 text-sm w-full"
                                    />
                                  </div>
                                  <div>
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {dbUsers.map((u) => (
                                        ...
                                        >
                                          <input
                                            type="checkbox"
                                            checked={editTaskAssignees.includes(u.email)}
                                            onChange={(e) => {
                                              const set = new Set(editTaskAssignees);
                                              e.target.checked
                                                ? set.add(u.email)
                                                : set.delete(u.email);
                                              setEditTaskAssignees(Array.from(set));
                                            }}
                                            className="w-3 h-3"
                                          />
                                          {u.name.split(' ')[0]}
                                  ...
                                  <div className="flex gap-2 ml-auto">
                                    ...
                                    <button
                                      onClick={cancelEdit}
                                      className="bg-slate-200 text-slate-600 px-3 py-1 rounded text-xs font-bold"
                                    >
                                      Cancel
                                    </button>
                                  </div>
          ...
          <div className="flex flex-col gap-4 p-4 md:p-6 overflow-y-auto custom-sidebar-scroll h-full">
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="Search all logs (user, action, content)..."
                className="flex-1 border border-slate-200 rounded-xl px-4 py-2 text-sm font-bold"
              />
              {logSearchQuery && (
                <button
                  onClick={() => setLogSearchQuery('')}
                  className="bg-slate-200 text-slate-600 px-3 py-2 rounded-xl text-sm font-bold"
                >
                  Clear
                </button>
              )}
                    ...
                    <tr>
                      <th className="px-3 py-3">
                        <input
                          type="checkbox"
                          onChange={() => {
                            if (selectedLogs.size === universalLogs.length)
                              setSelectedLogs(new Set());
                            else
                              setSelectedLogs(
                                new Set(universalLogs.map((l) => l.id))
                              );
                          }}
                          checked={
                            selectedLogs.size === universalLogs.length &&
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {universalLogs.map((log, idx) => (
                      <tr key={log.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-3">
                          <span
                            className={`text-[10px] font-extrabold tracking-wider px-2 py-1 rounded-full shadow-sm ${
                              log.type?.startsWith('TASK_')
                        ...
                        <td className="px-3 py-3 max-w-xs">
                          <div className="text-slate-600 whitespace-pre-wrap leading-relaxed font-medium text-[13px]">
                            {stripHtml(log.content)}
                          </div>
                          {log.target && (
                            <div className="text-[10px] font-bold text-indigo-500 mt-1 uppercase tracking-wider">
                              {stripHtml(log.target)}
                            </div>
                          )}
                        </td>
                      </tr>
              ...
              <div className="flex items-center gap-4 mb-6 border-b border-slate-100 pb-4 shrink-0">
                ...
                <div>
                  ...
                  <span className="text-[11px] font-bold text-rose-500 uppercase tracking-wider">
                    Push alerts to all active screens
                  </span>
                </div>
              </div>
                      ...
                      <div className="overflow-y-auto custom-sidebar-scroll flex-1 p-2">
                        {(displayedAcks || []).length === 0 ? (
                          ...
                          (displayedAcks || []).map((ack, i) => (
                            <div
                              key={i}
                              className="flex justify-between items-center px-3 py-2.5 hover:bg-slate-50 border-b border-slate-50 last:border-0 rounded"
                            >
                              <span className="text-sm font-bold text-slate-800">
                                <i className="fa-solid fa-check-double text-emerald-500 mr-2 text-[10px]"></i>
                                {ack.userName}
                              </span>
                              <span className="text-[11px] text-slate-400 font-bold">
                                {ack.timestamp?.toDate
                                  ? new Date(ack.timestamp.toDate()).toLocaleString()
                                  : ''}
                              </span>
                            </div>
                          ))
                    ...
                    <div className="flex flex-wrap gap-3 mb-6">
                      ...
                      >
                        <input
                          ...
                          name="urgency"
                          value="emergency"
                          checked={broadcastType === 'emergency'}
                      ...
                      <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-wrap gap-4 items-center justify-between shrink-0">
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500 mt-1">
                            Audit read receipts from past alerts.
                          </p>
                        </div>
                        <select
                          value={selectedBroadcastId || ''}
                          onChange={(e) => setSelectedBroadcastId(e.target.value)}
                          className="w-full sm:w-64 p-2.5 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20"
                        >
                          <option value="">Select a past broadcast...</option>
                          {(uniqueBroadcastIds || []).map((id) => (
                            <option key={id} value={id}>
                              Broadcast ID: {(id || '').substring(0, 6).toUpperCase()}
                            </option>
                          ))}
                        </select>
                      </div>
                        ...
                        {!selectedBroadcastId ? (
                          <div className="flex flex-col items-center justify-center h-full opacity-50 py-10">
                            ...
                            <p className="text-xs font-bold text-slate-500">
                              Select a broadcast to view its logs.
                            </p>
                          </div>
                        ) : (displayedAcks || []).length === 0 ? (
                          <div className="text-center text-slate-400 text-xs py-10 font-medium italic">
                            No acknowledgements found for this broadcast.
                          </div>
                        ) : (
                          (displayedAcks || []).map((ack, i) => (
                            <div
                              key={i}
                              className="flex justify-between items-center px-4 py-3 hover:bg-slate-50 border-b border-slate-50 last:border-0 rounded-lg transition-colors"
                            >
                              <span className="text-sm font-bold text-slate-800">
                                <i className="fa-solid fa-check-double text-emerald-500 mr-2 text-[10px]"></i>
                                {ack.userName}
                              </span>
                              <span className="text-[11px] text-slate-400 font-bold bg-white px-2 py-1 border border-slate-100 rounded-md shadow-sm">
                                {ack.timestamp?.toDate
                                  ? new Date(ack.timestamp.toDate()).toLocaleString()
                                  : ''}
                              </span>
          ...
          <div className="p-4 md:p-6 overflow-y-auto custom-sidebar-scroll h-full">
            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6 flex flex-col h-full min-h-[600px] overflow-hidden">
              <div className="flex items-center gap-4 mb-6 border-b border-slate-100 pb-4 shrink-0">
                ...
                <div>
                  <h2 className="font-bold text-slate-800 text-xl leading-tight">
                    Universal Tag Studio
                  </h2>
                  <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                    Manage Official Workflow Metadata
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 h-full min-h-0">
                <div className="w-full md:w-1/3 bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-inner overflow-y-auto custom-sidebar-scroll">
                  ...
                  <input
                    ...
                    placeholder="#Example"
                    className="w-full p-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-indigo-500/20 mb-5 font-bold text-slate-700 shadow-sm bg-white"
                  />
                  <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest block mb-2">
                    Select Theme
                  </label>
                  <div className="flex flex-wrap gap-2 mb-6 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                    {Object.entries(tagThemes).map(([name, classes]) => (
                      <div
                        key={name}
                        onClick={() => setNewTagTheme(name)}
                        className={`w-8 h-8 rounded-full cursor-pointer flex items-center justify-center shadow-sm border-2 transition-transform ${
                          newTagTheme === name
                  ...
                  <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 bg-white w-fit mb-6 shadow-sm">
                    <span
                      className={`px-2 py-1 rounded-md text-[11px] font-extrabold tracking-wide shadow-sm ${tagThemes[newTagTheme].bg} ${tagThemes[newTagTheme].text}`}
                    >
                      {newTagLabel || '#Preview'}
                    </span>
                  ...
                  <button
                    ...
                    className="w-full bg-indigo-600 text-white font-bold py-3.5 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-600/30 transition-all hover:-translate-y-0.5"
                  >
                    <i className="fa-solid fa-cloud-arrow-up mr-2"></i>Publish Tag
                  </button>
                </div>
                ...
                <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-5 overflow-y-auto custom-sidebar-scroll shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {(!customTags || customTags.length === 0) && (
                      <div className="col-span-full text-center py-10 text-slate-400 font-medium italic">
                        No tags created yet.
                      </div>
                    )}
                    {(customTags || []).map((tag) => (
                      <div
                        key={tag.id}
                        className="flex justify-between items-center p-3.5 border border-slate-100 rounded-xl bg-white shadow-sm hover:border-indigo-200 hover:shadow transition-all group"
                      >
                        <span
                          className={`px-2.5 py-1 rounded-md font-extrabold text-[11px] shadow-sm tracking-wide ${tag.bgClass} ${tag.textClass}`}
                        >
                          {tag.label}
                        </span>
                        <button
                          onClick={() => deleteDoc(doc(db, 'workspace_tags', tag.id))}
                          className="text-slate-300 hover:text-rose-500 w-8 h-8 flex items-center justify-center rounded-full hover:bg-rose-50 transition-colors opacity-0 group-hover:opacity-100"
                        >
