          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            {showAddUser && (
              <div className="p-5 bg-gray-50 border-b border-gray-200 flex flex-wrap gap-4 items-end">
                ...
                <button onClick={handleAddUser} className="bg-success text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-teal-600">Save User</button>
                ...
                <tbody className="divide-y divide-gray-100">
                  {dbUsers.map((u, idx) => (
                    <tr key={u.uid} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-3"><button onClick={() => handleToggleApprove(u)} className={`px-2.5 py-1 rounded-full text-[11px] font-bold transition-colors ${u.isApproved ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-orange-50 text-orange-700 border border-orange-200'}`}>{u.isApproved ? 'APPROVED' : 'PENDING'}</button></td>
              ...
              {groups.map(g => (
                <div key={g.id} className="bg-surface border border-gray-200 rounded-xl p-4 shadow-sm">
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <button onClick={() => { setActiveModal('group_settings'); /* also set activeGroup to this group */ }} className="flex-1 bg-gray-100 text-text-secondary py-2 rounded-lg text-xs font-bold hover:bg-gray-200"><i className="fa-solid fa-users mr-1"></i>Members</button>
                    {g.isArchived ? (
                      <button onClick={() => { if (window.confirm('Recover this team?')) handleAdminRecoverGroup(g.id, g.name); }} className="flex-1 bg-teal-50 text-teal-700 py-2 rounded-lg text-xs font-bold hover:bg-teal-100"><i className="fa-solid fa-rotate-left mr-1"></i>Recover</button>
                    ) : (
                      <button onClick={() => { if (window.confirm('Archive this team?')) handleAdminArchiveGroup(g.id, g.name); }} className="flex-1 bg-red-50 text-red-600 py-2 rounded-lg text-xs font-bold hover:bg-red-100"><i className="fa-solid fa-box-archive mr-1"></i>Archive</button>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
              <div className="flex flex-wrap gap-4">
                <div className="flex-1 min-w-[150px]"><label className="text-[10px] font-bold text-text-secondary uppercase tracking-wider block mb-1.5">By User</label>
                  <select value={adminFilterUser} onChange={e => setAdminFilterUser(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 outline-none focus:ring-2 focus:ring-primary/20"><option value="">All Users</option>{dbUsers.map(u => <option key={u.uid} value={u.email}>{u.name}</option>)}</select></div>
                <div className="flex-1 min-w-[150px]"><label className="text-[10px] font-bold text-text-secondary uppercase tracking-wider block mb-1.5">By Type</label>
                  <select value={adminFilterType} onChange={e => setAdminFilterType(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 outline-none focus:ring-2 focus:ring-primary/20"><option value="">All Types</option><option value="MESSAGE_CREATE">Public Messages</option><option value="MESSAGE_EDIT">Edited Messages</option><option value="MESSAGE_DELETE">Deleted Messages</option><option value="REACTION">Reactions</option><option value="TASK_CREATE">Tasks Created</option><option value="TASK_DELEGATE">Tasks Delegated</option><option value="TASK_COMPLETE">Tasks Completed</option><option value="TASK_COMMENT">Tasks Commented</option><option value="GROUP_ARCHIVE">Group Archived</option><option value="GROUP_RECOVER">Group Recovered</option></select></div>
                <div className="flex-1 min-w-[150px]"><label className="text-[10px] font-bold text-text-secondary uppercase tracking-wider block mb-1.5">By Group</label>
                  <select value={adminFilterGroup} onChange={e => setAdminFilterGroup(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 outline-none focus:ring-2 focus:ring-primary/20"><option value="">All Groups</option>{groups.map(g => <option key={g.id} value={g.id}>{g.name} {g.isArchived ? '(Archived)' : ''}</option>)}</select></div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-bold text-text-primary text-lg"><i className="fa-solid fa-diagram-project text-primary mr-2"></i>Organisation Task Tree</h2>
                  ...
                  Object.entries(tasksByGroup).map(([groupId, tasks]) => {
                    ...
                    return (
                      <div key={groupId} className="border border-gray-200 rounded-xl overflow-hidden">
                        <button onClick={() => setExpandedGroups(prev => ({ ...prev, [groupId]: !isExpanded }))} className="w-full flex items-center justify-between px-5 py-4 bg-gray-50 hover:bg-gray-100 transition-colors">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs bg-white text-text-secondary px-2 py-0.5 rounded-full border">{tasks.length} task{tasks.length !== 1 ? 's' : ''}</span>
                          </div>
                        </button>
                        {isExpanded && (
                          <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {tasks.map((task, idx) => (
                              <div key={task.id} onClick={() => openTaskTrail(task)} className="bg-surface border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                                <div className="flex items-center gap-2 mb-3">
                                  <span className="text-xs font-bold text-text-secondary bg-white px-2 py-0.5 rounded-full border">{idx + 1}</span>
                                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${task.taskData?.status === 'Completed' ? 'bg-teal-100 text-teal-700' : task.taskData?.status === 'In Progress' ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'}`}>{task.taskData?.status || 'Pending'}</span>
                                  {task.taskData?.priority && <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${task.taskData.priority === 'High' ? 'bg-red-100 text-red-700' : task.taskData.priority === 'Medium' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>{task.taskData.priority}</span>}
                                </div>
                                ...
                                <div className="flex items-center justify-between text-[11px] text-text-secondary">
                                  <div className="flex items-center -space-x-1.5">{(task.taskData?.assignees || []).slice(0, 3).map(email => { const assignee = dbUsers.find(u => u.email === email); return <MemoizedAvatar key={email} uid={assignee?.uid || email} url={assignee?.profilePicUrl} name={assignee?.name || email.split('@')[0]} sizeClass="w-6 h-6" extraClasses="border border-white" />; })}</div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
              ...
              <button onClick={printSelectedPDF} className="bg-primary text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-primary-hover">Print Selected Tasks</button>
            </div>
          </div>
        )}
        ...
        {activeTab === 'history' && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-5 border-b border-gray-100">
              <h2 className="font-bold text-text-primary text-lg flex items-center gap-2"><i className="fa-solid fa-clock-rotate-left text-primary mr-2"></i>User Activity History</h2>
            </div>
            <div className="p-5 bg-gray-50 border-b border-gray-200">
              <div className="flex flex-wrap gap-4 items-end">
                <div className="flex-1 min-w-[200px]">
                  ...
                  <select value={historyUserEmail} onChange={e => setHistoryUserEmail(e.target.value)} className="w-full p-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 outline-none focus:ring-2 focus:ring-primary/20">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {userHistoryLogs.length === 0 && <tr><td colSpan={4} className="px-5 py-8 text-center text-text-secondary italic">{historyUserEmail ? 'No activity found for this user.' : 'Select a user above.'}</td></tr>}
                  {userHistoryLogs.map((log, idx) => (
                    <tr key={log.id} className="hover:bg-gray-50">
                      <td className="px-5 py-3 text-text-secondary">{idx + 1}</td>
                      <td className="px-5 py-3"><div className="text-xs text-text-secondary">{log.dateString}</div><div className="text-[11px] text-text-secondary">{log.time}</div></td>
