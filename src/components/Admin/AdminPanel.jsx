...
export default function AdminPanel({
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'organizations', orgId, 'org_details', 'details'), (docSnap) => {
      if (docSnap.exists()) {
  ...
  useEffect(() => {
    if (!orgId) return undefined;
    const unsubs = [
      onSnapshot(collection(db, 'organizations', orgId, 'sessions'), (snap) => setSessions(snap.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => (b.lastActivity?.toMillis?.() || 0) - (a.lastActivity?.toMillis?.() || 0)))),
      onSnapshot(collection(db, 'organizations', orgId, 'roles'), (snap) => {
      ...
      onSnapshot(collection(db, 'organizations', orgId, 'retentionPolicies'), (snap) => setRetentionPolicies(snap.docs.map((d) => ({ id: d.id, ...d.data() })))),
      onSnapshot(query(collection(db, 'organizations', orgId, 'retention_cleanup_logs'), orderBy('timestamp', 'desc')), (snap) => setRetentionRuns(snap.docs.map((d) => ({ id: d.id, ...d.data() })))),
      onSnapshot(query(collection(db, 'organizations', orgId, 'exports'), orderBy('timestamp', 'desc')), (snap) => setExportsHistory(snap.docs.map((d) => ({ id: d.id, ...d.data() })))),
      onSnapshot(doc(db, 'organizations', orgId, 'settings', 'institution'), (snap) => {
  ...
  useEffect(() => {
    ...
    DEFAULT_ROLES.forEach((role) => {
      setDoc(doc(db, 'organizations', orgId, 'roles', role.id), { ...role, updatedAt: serverTimestamp() }, { merge: true }).catch(() => {});
    });
  }, [orgId]);

  // Broadcast acks (for live & historical)
  useEffect(() => {
    if (activeTab === 'broadcast' && orgId) {
      const unsubAcks = onSnapshot(collection(db, 'organizations', orgId, 'broadcast_acks'), (snap) => {
      ...
      const q = query(collection(db, 'organizations', orgId, 'broadcasts'), orderBy('timestamp', 'desc'));
  ...
  const updateRolePermission = async (role, area, action, checked) => {
    ...
    await setDoc(doc(db, 'organizations', orgId, 'roles', role.id), { ...role, permissions, updatedAt: serverTimestamp() }, { merge: true });
  ...
  const createRole = async () => {
    ...
    await setDoc(doc(db, 'organizations', orgId, 'roles', id), { id, name, system: false, permissions: makePermissionGrid(false), createdAt: serverTimestamp() });
  ...
  const deleteRole = async (role) => {
    ...
    await deleteDoc(doc(db, 'organizations', orgId, 'roles', role.id));
  ...
  const forceLogoutSession = async (session) => {
    ...
    await deleteDoc(doc(db, 'organizations', orgId, 'sessions', session.id));
  ...
  const forceLogoutAll = async () => {
    ...
    await Promise.all(sessions.map((session) => deleteDoc(doc(db, 'organizations', orgId, 'sessions', session.id))));
  ...
  const saveRetentionPolicy = async () => {
    ...
    await addDoc(collection(db, 'organizations', orgId, 'retentionPolicies'), payload);
    await logAuditEvent('RETENTION_POLICY_UPDATE', payload.category, payload);
  };

  const updateRetentionPolicy = async (policy, updates) => {
    await updateDoc(doc(db, 'organizations', orgId, 'retentionPolicies', policy.id), { ...updates, updatedAt: serverTimestamp() });
  ...
  const deleteRetentionPolicy = async (policy) => {
    ...
    await deleteDoc(doc(db, 'organizations', orgId, 'retentionPolicies', policy.id));
  ...
  const runCleanupNow = async (policy) => {
    ...
    const expiredMessages = (messages || [])
      ...
      .slice(0, 500);
    const runRef = await addDoc(collection(db, 'organizations', orgId, 'retention_cleanup_logs'), { ruleId: policy.id, ruleName: category, timestamp: serverTimestamp(), status: 'running', affected: 0 });
    for (const message of expiredMessages) {
      if (policy.action === 'archive') {
        const archiveCollection = message.isTask ? 'archived_tasks' : 'archived_messages';
        await setDoc(doc(db, 'organizations', orgId, archiveCollection, message.id), { ...message, archivedAt: serverTimestamp(), lifecycleRuleId: policy.id });
      }
      await deleteDoc(doc(db, "organizations", orgId, "messages", message.id));
    }
    await updateDoc(doc(db, 'organizations', orgId, 'retention_cleanup_logs', runRef.id), { status: 'completed', affected: expiredMessages.length });
  ...
  const exportFullDatabase = async () => {
    ...
    await addDoc(collection(db, 'organizations', orgId, 'exports'), { fileName, size: JSON.stringify(payload).length, timestamp: serverTimestamp(), status: 'ready', downloadUrl: 'Generated in browser download' });
  ...
  const executeHardDelete = async () => {
    ...
    await Promise.all(sessions.filter((s) => s.uid === selectedDsarUser.uid).map((s) => deleteDoc(doc(db, 'organizations', orgId, 'sessions', s.id))));
  ...
  const saveInstitutionSettings = async () => {
    ...
    await setDoc(doc(db, 'organizations', orgId, 'settings', 'institution'), payload, { merge: true });
  ...
  const publishBroadcast = async () => {
    ...
    if (!orgId) return alert('Organization context is still loading. Please try again.');
    setIsBroadcasting(true);
    // Store in current announcement
    await setDoc(doc(db, 'organizations', orgId, 'workspace', 'announcement'), {
    ...
    await addDoc(collection(db, 'organizations', orgId, 'broadcasts'), {
      message: broadcastMessage.trim(),
      type: broadcastType,
      timestamp: serverTimestamp(),
      author: currentUserData?.name || 'Administrator',
  ...
  const revokeBroadcast = async () => {
    ...
    if (!orgId) return;
    await updateDoc(doc(db, 'organizations', orgId, 'workspace', 'announcement'), { isActive: false });
  ...
  const saveOrgDetails = async () => {
    try {
      if (!orgId) throw new Error('Organization context is still loading.');
      const payload = { ...orgDetails, updatedAt: serverTimestamp() };
      await setDoc(doc(db, 'organizations', orgId), payload, { merge: true });
      await setDoc(doc(db, 'organizations', orgId, 'org_details', 'details'), payload, { merge: true });
      setIsOrgSaved(true);
  ...
  const printSelectedPDF = () => {
    if (activeTab === 'logs' && !hasFeature('auditLogs')) return;
  ...
  useEffect(() => {
    if (!adminTabs.includes(activeTab)) setActiveTab(adminTabs[0] || 'users');
  }, [activeTab, adminTabs]);
    ...
    <div className="flex flex-col h-full w-full bg-slate-50 overflow-hidden animate-in fade-in z-40 relative">
      ...
      <div className="bg-indigo-600 px-4 py-3 flex items-center justify-between shadow-md shrink-0 safe-top">
        ...
        <div className="flex items-center gap-2">
          {(['users', 'logs'].includes(activeTab) && (activeTab !== 'logs' || hasFeature('auditLogs'))) && (
            <button
      ...
      <div className="flex-1 overflow-hidden bg-slate-50 relative flex flex-col">
        ...
        {activeTab === 'overview' && hasFeature('advancedAnalytics') && (
          <div className="flex h-full flex-col gap-4 p-4 md:p-5 overflow-y-auto custom-sidebar-scroll">
            ...
            {hasFeature('auditLogs') && <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    {groups.map((g) => (
                      <tr key={g.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-right">
                          ...
                          <button onClick={async () => { if (!window.confirm('Delete this department?')) return; await deleteDoc(doc(db, 'organizations', orgId, 'groups', g.id)); }} className="text-rose-500 hover:text-rose-700" title="Delete"><i className="fa-solid fa-trash"></i></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
